-- Pin Qualifying statistic requirements in immutable package items while
-- preserving the existing optional-by-default creation contract.
...
create or replace function public.create_qualifying_session_draft_with_statistics(
  ...
  input_statistics_required boolean default false
)
returns uuid
language plpgsql
security invoker
...
declare
  ...
  required_state boolean := coalesce(input_statistics_required, false);
begin
    ...
    and not exists (
      ...
      where not exists (
        ...
        where item.package_version_id = version.id
          ...
          and item.is_required = required_state
      )
    )
  ...
  if target_package_version_id is null then
    ...
    select target_package_version_id, owner, selected.definition_version_id, selected.item_order - 1, required_state
...
revoke all on function public.create_qualifying_session_draft_with_statistics(text,text,text,jsonb,jsonb,jsonb,uuid[],boolean) from public, anon;
grant execute on function public.create_qualifying_session_draft_with_statistics(text,text,text,jsonb,jsonb,jsonb,uuid[],boolean) to authenticated;
...
set search_path = public
as $$
  select public.create_qualifying_session_draft_with_statistics(
    input_name,
    input_roster_type,
    input_scoring_mode,
    ...
    input_groups,
    input_days,
    input_statistic_definition_version_ids,
    false
  );
$$;
...
-- Signed-out leaderboard readers may learn only whether their already-valid
-- Tournament share token belongs to a Qualifying-backed Tournament.
create or replace function public.is_qualifying_backing_tournament(target_tournament_id uuid)
returns boolean
language sql
stable
security definer
set search_path = public
as $$
  select public.has_valid_share_token(
...
revoke all on function public.is_qualifying_backing_tournament(uuid) from public;
grant execute on function public.is_qualifying_backing_tournament(uuid) to anon, authenticated;

comment on function public.is_qualifying_backing_tournament(uuid) is
  'Returns only Qualifying backing context after validating the caller share token.';
