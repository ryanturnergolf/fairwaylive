...
test("Tournament Players step manages stable team drafts and remains contained on mobile", async ({ page }) => {
  ...
  const teamA = dialog.getByRole("region", { name: "Team A" });
  await expect(teamA).toBeVisible();
  await expect(teamA.getByText("0/5 players")).toBeVisible();
  await expect(teamA.getByRole("list", { name: "Team A roster slots" }).getByRole("listitem")).toHaveCount(5);
  await expect(teamA.getByText("Empty roster spot")).toHaveCount(5);
  await teamA.getByText("Team A", { exact: true }).click();
  await expect(teamA.getByRole("list", { name: "Team A roster slots" })).toBeHidden();
  await teamA.getByText("Team A", { exact: true }).click();
  await dialog.getByRole("button", { name: "Add Team" }).click();
  await dialog.getByRole("button", { name: "Add Team" }).click();
  await dialog.getByRole("button", { name: "Add Team" }).click();
  const teamD = dialog.getByRole("region", { name: "Team D" });
  await expect(teamD).toBeVisible();
  await expect(teamD.getByText("0/5 players")).toBeVisible();
  await expect(teamD.getByRole("list", { name: "Team D roster slots" })).toBeHidden();
  ...
  await dialog.getByRole("region", { name: "Team C" }).getByText("Team C", { exact: true }).click();
